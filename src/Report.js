...
export default class Report {
  ...
  async sendThreeHistoryTrack (socket, req) {
    ...
    let rows = null
    let sql = `SELECT hd.id, hd.obj_id, hd.card_type_id, hd.ident, hd.record_time, hd.map_id, hd.area_id, hd.reader_id, hd.ant, hd.tof_dist FROM ${dataTable} hd LEFT JOIN ${baseTable} bt on hd.obj_id=${indenkey} WHERE ${indenkey}=${objID}  AND record_time >= "${startTime}" AND record_time <= "${endTime}" ORDER BY record_time;`
    try {
      rows = await mysql.query(sql)
    } catch (err) {
